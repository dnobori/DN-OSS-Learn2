...
our %config = (
    ...
    "openssldir" => "",
    "options" => "--debug -static enable-threads enable-weak-ssl-ciphers enable-ssl3 enable-ssl3-method no-acvp-tests no-afalgeng no-asan no-async no-buildtest-c++ no-capieng no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-dso no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-engine no-external-tests no-fips no-fips-securitychecks no-fuzz-afl no-fuzz-libfuzzer no-ktls no-loadereng no-md2 no-module no-msan no-padlockeng no-pic no-rc5 no-sctp no-shared no-tests no-threads no-trace no-ubsan no-unit-test no-uplink no-zlib no-zlib-dynamic",
    "patch" => "0",
...
",
    ...
    "ldresflag" => " ",
    "lflags" => "",
    "lib_cflags" => "/Z7 /Fdossl_static.pdb /MTd /Z7",
    "lib_cppflags" => "",
    "lib_defines" => [
        "L_ENDIAN"
    ],
...
unless (caller) {
    ...
    use lib 'C:/git/SE-Build-crosslib_win32_v2/OpenSSL/src/build/vc2017/x64_Debug/util/perl';
    use OpenSSL::fallback 'C:/git/SE-Build-crosslib_win32_v2/OpenSSL/src/build/vc2017/x64_Debug/external/perl/MODULES.txt';
    ...
    if (scalar @ARGV == 0) {
        ...
        use lib '.';
        use lib 'C:/git/SE-Build-crosslib_win32_v2/OpenSSL/src/build/vc2017/x64_Debug/Configurations';
        use gentemplate;
